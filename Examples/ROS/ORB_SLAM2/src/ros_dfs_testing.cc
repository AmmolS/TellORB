...
vector<geometry_msgs::Point> dfs_all_destinations; 
		...
		if (isValid(pt.x, pt.y) 
			...
			{   
				...
				if(within_distance(init_x,init_y,pt.x,pt.y))
					...
					{	cout << "adding to all destinations list" << pt.x << pt.y << endl;
						//adding to all destinations
						dfs_all_destinations.push_back(pt); 				
					}	
				
				//return once destination list has the first node?
				//to prevent crashing
				if(dfs_all_destinations.size()==1000){
					//once dfs is done
					//choose the first one as the destination
								
					dfs_destinations.push_back(dfs_all_destinations[0]);
					dfs_destinations_visual.push_back(dfs_all_destinations[0]);
					return;
				}
...
void showGridMap(unsigned int id) {
	...
	if (show_camera_location) {
		...
		cv::Scalar destination_color(0,255,0);//color of choosen dfs destinations 
		...
		cv::Scalar all_destination_color(255,255,0);//color of all dfs destinations 
		for (int i = 0; i < dfs_all_destinations.size(); i++)  {
				cv::circle(grid_map_rgb, cv::Point((dfs_all_destinations[i].x)*resize_factor, (dfs_all_destinations[i].y)*resize_factor),
							3, all_destination_color, -1);
		}	
		dfs_all_destinations.clear();
